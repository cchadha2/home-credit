...
def installments(df):
    ...
    ins['EARLY_INSTALMENT_PAST_DUE'][(ins['NUM_INSTALMENT_NUMBER'] <= 3) & (ins['DUE_DATE_DIFF'] <= 0)]  = 1
    ins['EARLY_INSTALMENT_BEFORE_DUE'] = 0
    ins['EARLY_INSTALMENT_BEFORE_DUE'][(ins['NUM_INSTALMENT_NUMBER'] <= 3) & (ins['DUE_DATE_DIFF'] > 0)]  = 1
...
def prev_app(df):
    ...
    num_aggregations = {
        ...
        'DAYS_LENGTH': ['min', 'max', 'mean'],
        'DAYS_LENGTH_1ST_VERSION': ['min', 'max', 'mean'],
        'DAYS_LENGTH_TERMINATION': ['min', 'max', 'mean'],
        'NUM_INSTALMENTS': ['min', 'max', 'mean'],
        'NUM_INSTALMENTS_1ST_VERSION': ['min', 'max', 'mean'],
        'NUM_INSTALMENTS_TERMINATION': ['min', 'max', 'mean'],
...
def remove_on_importance(df, remove_by_threshold = False, remove_by_rank = False):
    ...
    importance = pd.read_csv('../output/importance_3.4.csv')
...
def main(
    ...
    df = pd.read_csv('../data/processed_data_3.4.csv', compression = 'zip')
    ...
    df.to_csv('../data/processed_data_3.4.csv', compression = 'zip')  
