...
def bureau_and_balance(df):
    ...
    df['BURO_CNT'] = df['previous_loans']
    df = df.drop('previous_loans', axis=1)
    ...
    df['BURO_COMPLETED_CNT'] = df['closed_loans']
    df = df.drop('closed_loans', axis=1)
    ...
    bureau['ACTIVE_LOAN_TERM'] = (bureau['DAYS_CREDIT_ENDDATE'] - bureau['DAYS_CREDIT'])/30 
    bureau['CLOSED_LOAN_TERM'] = (bureau['DAYS_ENDDATE_FACT'] - bureau['DAYS_CREDIT'])/30 
    ...
    bureau_aggregations = {'DAYS_CREDIT':['min', 'max', 'mean', 'var'],
                        ...
                        #    'ACTIVE_LOAN_TERM':['min', 'max', 'mean', 'sum', 'var'],
                        #    'CLOSED_LOAN_TERM':['min', 'max', 'mean', 'sum', 'var'],
                           }
...
def cc_balance(df):
    ...
    df['CC_COMPLETED_CNT'] = df['NAME_CONTRACT_STATUS_Completed'].fillna(0)
    df = df.drop('NAME_CONTRACT_STATUS_Completed', axis=1)
    ...
    df['CC_ACTIVE_CNT'] = df['NAME_CONTRACT_STATUS_Active'].fillna(0)
    df = df.drop('NAME_CONTRACT_STATUS_Active', axis=1)
...
def remove_on_importance(df, remove_by_threshold = False, remove_by_rank = False):
    ...
    importance = pd.read_csv('../output/importance_3.6.csv')
...
def main(
    ...
    df.to_csv('../data/processed_data_3.6_pruned.csv', compression = 'zip')  
